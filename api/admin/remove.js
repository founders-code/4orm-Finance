/* =====================================================================
   POST /api/admin/remove  { kind: 'email'|'domain', value }
   Removes an entry from the KV allow-list. Admin only.
   Existing sessions stay valid for up to 30 days; rotate SESSION_SECRET
   to force immediate revoke for everyone.
   ===================================================================== */
...
export default async function handler(req){
  ...
  if(!value) return json({ error: 'missing_value' }, 400);

  const key = kind === 'email' ? 'allowlist:emails' : 'allowlist:domains';
  const r = await kv(['SREM', key, value]);
  if(r.error) return json({ error: r.error }, 500);
  return json({ ok: true, removed: value, kind: kind });
}
